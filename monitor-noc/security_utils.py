...
# Tenta carregar biblioteca cryptography para AES-256 / Fernet
try:
...
def decrypt_secret(cipher_text: str) -> str:
    ...
    key = get_encryption_key()
    if not key or not HAS_CRYPTOGRAPHY:
        logger.warning("Tentativa de decriptar sem ENCRYPTION_KEY ou sem biblioteca cryptography.")
        return cipher_text
    ...
    try:
        ...
        logger.error(f"Falha ao decriptar segredo com a chave fornecida: {e}")
        return cipher_text

def sanitize_device_for_api(device: dict) -> dict:
    """
    Remove ou ofusca campos confidenciais (ex: senhas) antes de retornar na API pública/operador.
    """
    sanitized = dict(device)
    raw_pass = sanitized.get("password", "")
    sanitized["has_password"] = bool(raw_pass)
    # Remove senha em texto puro do payload de saída
