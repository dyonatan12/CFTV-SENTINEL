...
# --- UTILITÁRIOS DE HASHING DE SENHA (PBKDF2-HMAC-SHA256) ---

def hash_password(password: str, salt: Optional[str] = None) -> tuple[str, str]:
    """Retorna (password_hash, salt) usando PBKDF2 com 100.000 iterações."""
    if not salt:
...
def verify_password(password: str, hashed: str, salt: str) -> bool:
    """Verifica se a senha fornecida corresponde ao hash armazenado usando tempo constante."""
...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None) -> str:
    ...
    # Adiciona expiração e issued-at com margem de tolerância
    to_encode.update({
    ...
    if HAS_PYJWT:
        ...
        # Fallback de JWT nativo HMAC-SHA256 caso PyJWT não esteja instalado
...
def decode_access_token(token: str) -> Optional[dict]:
    if HAS_PYJWT:
        try:
            ...
            logger.warning(f"Erro ao decodificar token JWT via PyJWT: {e}")
            return None
    else:
        # Validação do fallback nativo
        try:
            parts = token.split(".")
            if len(parts) != 3:
                return None
            ...
            logger.warning(f"Erro na validação do token nativo: {e}")
            return None
...
def require_role(allowed_roles: List[str]):
    """Gera dependência para verificar se o usuário autenticado possui uma das roles permitidas."""
